...
function VegItems() {
  ...
  const vegItems = useSelector((state) => state.products.vegItems);

  const itemsPerPage = 8; // 2 rows of 4 cards
  const [currentPage, setCurrentPage] = useState(1);

  if (!vegItems || vegItems.length === 0) {
    return <div className="container mt-4 text-center">No Veg Items Found.</div>;
  }
  ...
  const currentItems = vegItems.slice(startIndex, endIndex);

  return (
    <div className="container-fluid mt-4 px-3">
      {/* Heading */}
      <h3 className="mb-4 text-center">Welcome to Veg Page</h3>
